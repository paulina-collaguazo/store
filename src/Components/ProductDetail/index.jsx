...
const ProductDetail = () => {
    ...
    return(
        <aside className={`${context.isProductDetailOpen ? 'flex': 'hidden'} product-detail flex flex-col fixed right-0 border border-black rounded-lg bg-white`}>
            ...
            <figure>
                <img className='w-full h-full px-4 rounded-lg' src={context.selectedProduct.images[0].replace('["', '').replace('"', '')} alt={context.selectedProduct.title}/>
                <p className='flex flex-col p-6'>
                    <span className='font-medium text-2xl'>${context.selectedProduct.price}</span>
